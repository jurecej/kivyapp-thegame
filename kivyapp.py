...
class Igra(GridLayout):
    def __init__(self):
        ...
        self.label = Label(text='[size=80][b]THE GAME[/b][/size]', color=color1, font_size=28, halign='center', markup=True)
        self.add_widget(self.label)

        self.textinput1 = TextInput(text='Vpiši ime igralca tukaj.', size_hint=(.2, .3), font_size=36, multiline=False)
        self.add_widget(self.textinput1)

        self.button3 = Button(text='+ Dodaj igralca', font_size=56, size_hint=(.2, .3), background_color=color1, color=color2, on_press=self.textinput, markup=True)
        ...
        self.button2 = Button(text='Izhod', font_size=56, size_hint=(.2, .3), background_color=color1, color=color3, on_release=self.stop, markup=True)
